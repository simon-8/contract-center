...
var pickerEvent = {
    ...
    getLast: function () {
        ...
        showPricePickerWithDate(obj.year, obj.month);
    },
    getNext: function () {
        dateUtil.getNexDate();
        //pickerEvent.Init(elem);
        ...
        for (var i = 1; i <= 42; i++) {
            ...
            if ((i - 1) >= week && (i - c) <= days) {
                ...
                if (price != -1) {
                    ...
                    classStyle = "class='on'";
                    if (status != -1) {
                        classStyle = "class='on"+ (status  == 1 ? ' bg-success' :  ' bg-danger') +"'";
                    }
                }
...
var commonUtil = {
    ...
    getStatus: function (day) {
        ...
        for (var i = 0; i < obj.priceArr.length; i++) {
            if (obj.priceArr[i].day == date) {
                return parseInt(obj.priceArr[i].status);
            }
        }
        return -1;
    },
    chooseClick: function (sender) {
        ...
        if (input.html()) {
            ...
            $(sender).children().append("<span class='input'><input maxlength='4'></span>");
        ...
        $(sender).find('input').blur(function () {
            ...
            console.log(newPrice, typeof newPrice, parseInt(newPrice) ? 'on bg-success' : 'on bg-danger');
            $(sender).toggleClass(parseInt(newPrice) ? 'on bg-success' : 'on bg-danger');
            ...
            changePrice(date, newPrice, calendarPrice, function(res) {
                if (res) {
                    calendarPrice.html("<dfn>¥</dfn>" + $(sender).attr("price"));
                    //$(sender).val(newPrice);
                }
            });
